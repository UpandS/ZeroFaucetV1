...
public function createFaucetpayDeposit($userId, $amount, $currency) {
    // Először lekérjük a jelenlegi deposit értéket
    $stmt = $this->mysqli->prepare("SELECT deposit FROM users WHERE id = ?");
    $stmt->bind_param("i", $userId);
    $stmt->execute();
    $stmt->bind_result($currentDeposit);
    if ($stmt->fetch()) {
        $stmt->close();
        $newDeposit = $currentDeposit + $amount;

        // Frissítjük a deposit mezőt
        $stmt = $this->mysqli->prepare("UPDATE users SET deposit = ? WHERE id = ?");
        $stmt->bind_param("di", $newDeposit, $userId);
        $stmt->execute();
        $stmt->close();
    } else {
        // Hibakezelés, ha a user nem található
